                          ...
                          <div className="speak-row">
                            <button 
                              ...
                              onClick={(e) => {
                                e.preventDefault();
                                e.stopPropagation();
                                if (onSpeak && plainText) {
                                  console.log('[Speak Button] Clicked, text length:', plainText.length);
                                  onSpeak(plainText, m.id);
                                }
                              }} 
...
function MoleculeCard({ name, smiles, width = 220, height = 160 }) {
  ...
  useEffect(() => {
    // If a well-known molecule name is provided, prefer the curated SMILES
    // over any LLM-provided SMILES to avoid name↔smiles mismatches
    if (name) {
      const resolved = moleculeSmiles[name.toLowerCase()];
      if (resolved && resolved !== actualSmiles) {
...
function parseAssistantContent(raw) {
  ...
  // Clean text thoroughly for speech
  const plainText = answerPart
    .replace(/\[EVENT\].*$/g, '')                    // Remove [EVENT] markers
